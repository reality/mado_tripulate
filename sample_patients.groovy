...
def trains = new File('ohshit.txt').text.split('\n')
...
while(sampled_patients.size() < 2500) {
  ...
  if(new File('new_texts/' + key + '.txt').exists() && !already.contains(key) && !trains.contains(key)){
    sampled_patients << key + '\t' + pt.join(',')
    already << key
  }
}

new File('TEST_sampled_patient_visits.csv').text = sampled_patients.join('\n')
